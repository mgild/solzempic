...
pub fn to_json(
    ...
    for (i, instr) in instructions.iter().enumerate() {
        ...
        // 8-byte reserved discriminator: opcode in byte 0, zeros in bytes 1-7.
        // The dispatcher generated by `#[SolzempicEntrypoint]` reads byte 0 for
        // the opcode and skips to offset 8 for params, so the on-wire encoding
        // is `[opcode, 0, 0, 0, 0, 0, 0, 0]`. Emitting a single-byte
        // discriminator here causes Codama to generate a 1-byte encoder, which
        // drops the 7 pad bytes and shifts params to offset 1, producing
        // `InvalidInstructionData` at runtime.
        json.push_str(&format!(
